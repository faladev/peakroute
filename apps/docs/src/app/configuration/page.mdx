...
<table>
  ...
  <tbody>
    <tr>
      <td>`PEAKROUTE_PORT`</td>
      <td>Proxy port</td>
      <td>`1355`</td>
    </tr>
    <tr>
      <td>`PEAKROUTE_HTTPS`</td>
      <td>Set to `1` to always enable HTTPS</td>
      <td>off</td>
    </tr>
    <tr>
      <td>`PEAKROUTE_STATE_DIR`</td>
...
Override with `PEAKROUTE_STATE_DIR`.
...
Apps get a random port in the 4000--4999 range. Portless sets `PORT` and `HOST` before running your command. Most frameworks respect `PORT` automatically. For frameworks that ignore it (Vite, Astro, React Router, Angular), peakroute auto-injects `--port` and `--host` flags.
