...
Two projects default to the same port and you get `EADDRINUSE`. With peakroute, each app gets an auto-assigned port behind a named URL -- no collisions.

## Memorizing ports

Was the API on 3001 or 8080? With peakroute, it is always `http://api.localhost:1355`.
...
CORS allowlists, OAuth redirect URIs, and `.env` files all break when ports change. With peakroute, URLs are stable across restarts.

## Sharing URLs with teammates

"What port is that on?" becomes a Slack question. With peakroute, everyone uses the same named URL.
