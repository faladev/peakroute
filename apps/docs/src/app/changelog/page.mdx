...
- **sudo state directory permissions**: System state directory (`/tmp/peakroute`) now uses world-writable + sticky-bit permissions so non-root processes can register routes after a sudo proxy start. State files created under sudo are chowned back to the real user.
...
- **proxy loop detection**: Detect forwarding loops (e.g. a Vite dev server proxying back through peakroute without `changeOrigin: true`) using the `X-Portless-Hops` header. Respond with `508 Loop Detected` and a message explaining the fix. Also detects loops on WebSocket upgrades.
- **`--force` flag**: Override a route registered by another process with `peakroute <name> --force <cmd>`.
...
- `sudo peakroute trust` command for system CA trust
- `--https`, `--cert`, `--key`, `--no-tls` flags for proxy start
- HTTPS configuration via `peakroute.json` and `PEAKROUTE_HTTPS` env var

## 0.3.0

- Configuration file support (`peakroute.json`)
- `PEAKROUTE=0` / `PEAKROUTE=skip` to bypass the proxy
